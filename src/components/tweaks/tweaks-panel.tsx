...
import {
  ...
  isTweaksEnvironment,
  postToEditModeHost,
  shouldOpenTweaksFromQuery,
} from '@/lib/edit-mode-host';
...
export function useTweaks<T extends Record<string, unknown>>(defaults: T) {
  ...
  const setTweak = useCallback(<K extends keyof T>(key: K, val: T[K]) => {
    ...
    postToEditModeHost({ type: '__edit_mode_set_keys', edits: { [key]: val } });
  }, []);
...
export function TweaksPanel({ title = 'Tweaks', children }: { title?: string; children: ReactNode }) {
  ...
  useEffect(() => {
    if (shouldOpenTweaksFromQuery()) setOpen(true);
  }, []);
  ...
  useEffect(() => {
    ...
    postToEditModeHost({ type: '__edit_mode_available' });
    return () => window.removeEventListener('message', onMsg);
  }, []);

  useEffect(() => {
    if (process.env.NODE_ENV !== 'development' || isEditModeHost()) return;
  ...
  const dismiss = () => {
    setOpen(false);
    postToEditModeHost({ type: '__edit_mode_dismissed' });
  };
  ...
  if (!isTweaksEnvironment() || !open) return null;
  return (
    <>
